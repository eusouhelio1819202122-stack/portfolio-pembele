...
// Executa o logout se solicitado
...
<body>
  ...
  <div style="display: flex; flex-direction: column; gap: 30px; padding: 10px;">
    ...
    <main class="form-container" style="margin-top: 20px; max-width: 650px;">
      ...
      <p style="color: var(--cor-cinza); font-size: 0.85rem; margin-bottom: 20px;">Altere qualquer texto do site institucional instantaneamente.</p>
      ...
      <form action="admin.php" method="POST">
        ...
        <div class="form-group">
          ...
          <input type="text" name="hero_titulo" class="form-input" value="<?php echo htmlspecialchars($config['hero_titulo'] ?? ''); ?>" required>
        </div>
        <div class="form-group">
          <label>Subtítulo Principal (Hero)</label>
          <textarea name="hero_subtitulo" class="form-input" style="height: 70px;" required><?php echo htmlspecialchars($config['hero_subtitulo'] ?? ''); ?></textarea>
        </div>
        <div class="form-group">
          <label>Título da Seção "Sobre Nós"</label>
          <input type="text" name="sobre_titulo" class="form-input" value="<?php echo htmlspecialchars($config['sobre_titulo'] ?? ''); ?>" required>
        </div>
        <div class="form-group">
          <label>Texto da Seção "Sobre Nós"</label>
          <textarea name="sobre_texto" class="form-input" style="height: 100px;" required><?php echo htmlspecialchars($config['sobre_texto'] ?? ''); ?></textarea>
        </div>
        <div class="form-group">
          <label>E-mail de Contacto</label>
          <input type="email" name="contacto_email" class="form-input" value="<?php echo htmlspecialchars($config['contacto_email'] ?? ''); ?>" required>
        </div>
        <div class="form-group">
          <label>Telefone de Exibição</label>
          <input type="text" name="contacto_telefone" class="form-input" value="<?php echo htmlspecialchars($config['contacto_telefone'] ?? ''); ?>" required>
        </div>
        <div class="form-group">
          <label>Número do WhatsApp (Apenas números com código do país. Ex: 244900000000)</label>
          <input type="text" name="contacto_whatsapp" class="form-input" value="<?php echo htmlspecialchars($config['contacto_whatsapp'] ?? ''); ?>" required>
        </div>
        <div class="form-group">
          <label>Localização (Cidade/País)</label>
          <input type="text" name="contacto_localizacao" class="form-input" value="<?php echo htmlspecialchars($config['contacto_localizacao'] ?? ''); ?>" required>
        </div>

        <button type="submit" class="btn-submit" style="background-color: #00e676; color: #000;">Salvar Alterações do Site</button>
      </form>
    </main>

    <main class="form-container" style="max-width: 650px;">
      <h2>Publicar Novo Trabalho no Portfólio</h2>
      <p style="color: var(--cor-cinza); font-size: 0.85rem; margin-bottom: 20px;">Carregue mídias visuais para a galeria de trabalhos recentes.</p>
      ...
      <form action="admin.php" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input type="text" name="servico" placeholder="Ex: Production de Vídeo, Branding" class="form-input" required>
        </div>
        <div class="form-group">
    ...
    <main class="form-container" style="max-width: 650px; margin-bottom: 60px;">
      ...
      <p style="color: var(--cor-cinza); font-size: 0.85rem; margin-bottom: 20px;">Selecione o projeto e envie fotos extras para completar as imagens de exibição.</p>
      ...
      <form action="admin.php" method="POST" enctype="multipart/form-data">
        ...
        <button type="submit" class="btn-submit" style="background-color: #fff; color: #000;">Adicionar Foto à Galeria</button>
      </form>
    </main>
